...
const string strMessageMagic = "ReviveCoin Signed Message:\n";
...
if(nBestHeight < 10000)
    ...
    {
           nSubsidy = nSubsidy * 105; // 105%
    }
else if(nHeight < 40000)
    {
           nSubsidy = nSubsidy * 110.250; // 110,250%
    }
else if(nHeight < 50000)
    {
           nSubsidy = nSubsidy * 115.763; // 115,763%
    }
else if(nHeight < 60000)
    {
           nSubsidy = nSubsidy * 121.551; // 121,551%
    }
else if(nHeight < 70000)
    {
           nSubsidy = nSubsidy * 127.628; // 127,628%
    }
else if(nHeight < 80000)
    {
           nSubsidy = nSubsidy * 134.010; // 134,010%
    }
    ...
    {
           nSubsidy = nSubsidy * 155.133; // 155,133%
    }
    ...
    {
           nSubsidy = nSubsidy * 179.586 ; // 179,586%
    }
else if(nHeight < 150000)
    {
           nSubsidy = nSubsidy * 188.586; // 188,586%
    }
else if(nHeight < 160000)
    {
           nSubsidy = nSubsidy * 197.993; // 197,993%
    }
else if(nHeight < 170000)
    {
           nSubsidy = nSubsidy * 207.893; // 207,893%
    }
else if(nHeight < 180000)
    {
           nSubsidy = nSubsidy * 218.287; // 218,287%
    }
    ...
    {
           nSubsidy = nSubsidy * 252.695; // 252,695%
    }
    ...
    {
           nSubsidy = nSubsidy * 292.526; // 292,526%
    }
else if(nHeight > 240001)
    {
           nSubsidy = nSubsidy * 307.152; // 307,152%
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ReviveCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "REVIVE COIN V0.1 14-10-2017";
        CTransaction txNew;
        txNew.nTime = 1507912777;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
